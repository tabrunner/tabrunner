    ...
    params: {
      ...
      properties: {
        ...
        deviates_from_approved: {
          type: "boolean",
          description:
            "Your judgment, not a diff: true only when the upcoming work could cost the user something they didn't sign up for — money spent, a message or post sent, data deleted or exposed, a new account, service, or destination — the kind of thing they'd want to veto before it happens. Weigh the cost of being wrong, not the size of the edit: rewording, reordering, splitting a step, a different page or route to the same approved end, marking progress, and dropping work are all false (doing less never exceeds the yes you already have). So is anything the user just asked for in a message — their message is its own approval — and so is re-sending approved work on a fresh run of this conversation. true parks the run for a fresh approval, so reserve it for changes worth the interruption.",
        },
      },
