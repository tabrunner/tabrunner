...
| Classifier executors (Jev, local models)    | Jev 1.13, tested 2026-09: right element 49/49 in 0.37s for ~$0.0008 a step, 3–6× faster and 22–57× cheaper than Claude. But a page the LLM planned already has its refs, so it saves only same-page turns, for a new vendor and key. In-browser SemIf-style models cap at 20 options. Revisit if most turns stay on one page. |
